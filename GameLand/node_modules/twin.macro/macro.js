...
function sassifyArbitraryVariants(fullClassName, {
  ...
  const variantsWithParentSelectors = collapsed.map((v, idx) => {
    ...
    const isMissingParentSelectorOkay = hasArbitraryVariant && hasNormalVariant && !v.includes('&');
    assert(!isMissingParentSelectorOkay, ({
      color
    }) => `${color(`✕ ${String(color(fullClassName, 'errorLight'))} had trouble with the auto parent selector feature`)}\n\nYou’ll need to add the parent selector manually within the arbitrary variant(s), eg: ${String(color(`[section &]:block`, 'success'))}`);
    hasArbitraryVariant = true;
    const out = addParentSelector(unbracket(v), collapsed[idx - 1], (_collapsed = collapsed[idx + 1]) != null ? _collapsed : '');
    return `[${out}]`;
  });
  return [...variantsWithParentSelectors, className].join((_tailwindConfig$separ7 = tailwindConfig.separator) != null ? _tailwindConfig$separ7 : ':');
}

function addParentSelector(rawSelector, prev, next) {
  // Tailwindcss requires pre-encoded commas - unencoded are removed and we end up with an invalid selector
  let selector = rawSelector.replace(ALL_COMMAS, '\\2c'); // Escape class dots in the selector - otherwise tailwindcss adds a the prefix within arbitrary variants (only when `prefix` is set in tailwind config)
  // eg: tw`[.a]:first:tw-block` -> `.tw-a &:first-child`

  selector = selector.replace(ALL_CLASS_DOTS, '\\.'); // Preserve selectors with a parent selector and media queries
  ...
  if (BASIC_SELECTOR_TYPES.test(selector) && !prev) return `& ${selector}`; // Pseudo elements get an auto parent selector prefixed

  if (selector.startsWith(':')) return `&${selector}`; // When there's more than one variant and it's at the end then prefix it

  if (!next && prev) return `&${selector}`; // When a non arbitrary variant follows then we combine it with the current
